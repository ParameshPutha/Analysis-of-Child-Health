# -*- coding: utf-8 -*-
"""
Created on Mon Mar 14 20:42:11 2022
...
#Importing Data
mode = pd.read_csv(r"C:\Users\DELL\Desktop\Project\df_behavioural.csv")

mode['BehaviouralScore'].replace('Expected', 0 , inplace=True)
mode['BehaviouralScore'].replace('Borderline', 1 , inplace=True)
mode['BehaviouralScore'].replace('Significant', 2 , inplace=True)
...
mode.drop(['AnotherChildHome','PeopleInHome','breakfast','GadgetInWeek',"PlayOutside",'SchoolCommunity','ChoicesOnThing','ImGoodAtThings','Health','School','Family','Friends', 'Appearance', 'Life'], axis=1, inplace=True)
d=mode.corr()
#mode.to_csv('df_behaviour10.csv', index=False) #exporting cleaned dataframe
'''
...
modell = LogisticRegression(C=C, penalty='l2',solver='saga',multi_class='multinomial',max_iter=10000).fit(train.iloc[:, :24], train.iloc[:, -1:])
...
'''
score=cross_val_score(modell,train.iloc[:, :10], train.iloc[:,-1:],cv=5,scoring = 'accuracy').mean()
print(score)
